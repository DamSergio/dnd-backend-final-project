...
async def edit_cantrips(req: Request, user: User = Depends(current_user)):
    """Edit the cantrips of a character."""
    if not user:
        return JSONResponse(
            status_code=401,
            content={
    ...
    character.abilities = data["cantrips"]
