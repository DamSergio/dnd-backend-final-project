...
async def edit_coins(req: Request, user: User = Depends(current_user)):
    """Edit the coins of a character."""
    if not user:
        return JSONResponse(
            status_code=401,
            content={
    ...
    character.coins.cp = data["cp"]
    character.coins.sp = data["sp"]
    character.coins.ep = data["ep"]
    character.coins.gp = data["gp"]
    character.coins.pp = data["pp"]
